export const quiz = {
	questions: [
		...
		{
			'question': 'Какое из полей объявлено с ошибкой на [картинке](http://telegram.funnycode.ga/img/ios/1.png)?',
		...
		{
			'question': `В примере на [картинке](http://telegram.funnycode.ga/img/ios/2.png) для компилятора достаточно, чтобы типы для generic-функции были выведены автоматически?
...
`,
		...
		{
			'question': 'Каким образом можно проверить, что класс на [картинке](http://telegram.funnycode.ga/img/ios/3.png) реализует функцию `call` протокола `Callable`?\n' +
						'\n*1.* `instance.call != nil`' +
		...
		{
			'question': 'Правильно ли определено перечисление на [картинке](http://telegram.funnycode.ga/img/ios/4.png)?',
...
`,
		...
		{
			'question': `Что будет выведено командой print на [картинке](http://telegram.funnycode.ga/img/ios/5.png)?
