export const quiz = {
	questions: [
		...
		{
			'question': 'Какое из полей объявлено с ошибкой на [картинке](http://telegram.funnycode.ga/img/ios/1.png?v=1)?',
			//'img': 'http://telegram.funnycode.ga/img/ios/1.png?v=1',
			'answer': 0,
			'variants': [
				'все корректно',
				'val1',
		...
		{
			'question': `В примере на [картинке](http://telegram.funnycode.ga/img/ios/2.png?v=1) для компилятора достаточно, чтобы типы для generic-функции были выведены автоматически?
...
`,
			//'img': 'http://telegram.funnycode.ga/img/ios/2.png?v=1',
			'answer': 0,
			'variants': [
				'1',
				'2',
		...
		{
			'question': 'Каким образом можно проверить, что класс на [картинке](http://telegram.funnycode.ga/img/ios/3.png?v=1) реализует функцию `call` протокола `Callable`?\n' +
						'\n*1.* `instance.call != nil`' +
						'\n\n*2.* `instance.respondsToSelector( #selector(Callable.call) )`' +
						'\n\n*3.* `if let _ = instance.call{ … }`' +
						'\n\n*4.* `instance.call?() == nil`',
			//'img': 'http://telegram.funnycode.ga/img/ios/3.png?v=1',
			'answer': 0,
			'variants': [
				'1 и 2',
				'2 и 3',
		...
		{
			'question': 'Правильно ли определено перечисление на [картинке](http://telegram.funnycode.ga/img/ios/4.png?v=1)?',
			'img': 'http://telegram.funnycode.ga/img/ios/4.png?v=1',
			'answer': 0,
			'variants': ['да', 'нет']
		},
		{
...
`,
		...
		{
			'question': `Что будет выведено командой print на [картинке](http://telegram.funnycode.ga/img/ios/5.png?v=1)?

*1.* init count = 2, array count = 4
*2.* init count = 4, array count = 4
*3.* не скомпилируется
`,
			//'img': 'http://telegram.funnycode.ga/img/ios/5.png?v=1',
			'answer': 0,
			'variants': [
				'1',
				'2',
