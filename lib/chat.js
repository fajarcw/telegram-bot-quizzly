			...
			if (!this.finish) {
				...
				sender.then($=> {
					sendMessage(
						...
						'\n\nМы попросим оставить нам твой номер телефона, чтобы мы могли оперативно связаться' +
						' с тобой в случае, если ты хорошо прошел наш тест',
						{
				...
				if (++this.finish < 3) {
					sendMessage(
						'Диалог окончен, спасибо что приняли участие =). Отключаюсь...',
						{
							reply_markup: JSON.stringify({
								inline_keyboard: [
									[{
