...
export default class Chat {
	...
	constructor(request, bot, fromDump = false) {
		...
		this.run = func=> {
			this.workdir = `log/${nowdate}/${chatId}`;
			this.loadQuestions();
...
//*/
		...
		if (this.counter.quiz.number.current === this.counter.quiz.number.total) {
			if (!this.finish) {
				...
				if (!sender)
					sender = sendMessage('Надеюсь, что понравились наши вопросы \n\n 🎉');
				...
				sender.then($=> {
					sendMessage(
						...
						' Повторное прохождение теста возможно уже завтра' +
						'\n\nА еще мы попросим оставить нам твой номер телефона, чтобы мы могли оперативно связаться' +
						' с тобой в случае, если ты выиграл',
						{
				...
				if (++this.finish < 3) {
					sendMessage('Диалог окончен, спасибо что приняли участие =). Отключаюсь...');
					this.log({phone: text});
				}
				else return;
			}
...
${quiz.img ? '(отправляю картинку, жди...)' : ''}
						`,
						quiz.data
					).then($=> {
						if (quiz.img) {
							bot
								.sendPhoto(chatId, quiz.img)
								.then($=>{
									sendMessage(
										'Если вдруг пропала клавиатура, просто нажми на значок `[::]`,' +
										' который находится возле поля ввода для текста ↘️',
										{parse_mode: 'Markdown'}
									)
								});
						}
